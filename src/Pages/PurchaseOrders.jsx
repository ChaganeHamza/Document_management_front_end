...
const PurchaseOrders = (props) => {
  ...
  useEffect(() => {
      const fetchData = async () => {
          try {
              const response1 = await axios.get('https://localhost:44330/api/getPurchaseOrders');
              console.log('Sales Orders Data:', response1.data);
              setPurchaseOrders(response1.data);
          } catch (error) {
  ...
  const filteredData = searchTerm.length === 0 ? PurchaseOrders : PurchaseOrders.filter(order =>
      columns.some(column =>
