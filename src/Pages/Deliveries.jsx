...
const Deliveries = (props) => {
  ...
  useEffect(() => {
      const fetchData = async () => {
          try {
              const response1 = await axios.get('https://localhost:44330/api/getDeliveries');
              console.log('Sales Orders Data:', response1.data);
              setDeliveries(response1.data);
          } catch (error) {
  ...
  const filteredData = searchTerm.length === 0 ? Deliveries : Deliveries.filter(order =>
      columns.some(column =>
